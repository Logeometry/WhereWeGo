  ...
  useEffect(() => {
    ...
    if (state.surveyAttractions) {
      setAvailablePlaces(state.surveyAttractions.map(attraction => ({
        ...
        locX: attraction.lng || (attraction.location?.coordinates?.[0]), // 위도, 경도 필드명에 유의 (백엔드는 [lng, lat] 배열)
        locY: attraction.lat || (attraction.location?.coordinates?.[1]),
      ...
      try {
        ...
        setDailySchedule(data.dailySchedule.map(day => ({
          ...
          places: day.places.map(place => ({
            id: place.id || place._id, // 백엔드의 id 또는 _id를 프론트엔드의 id로 사용
            name: place.name,
            description: place.description,
            duration: place.estimated_duration, // 백엔드의 estimated_duration을 duration으로 변환
            locX: place.location.coordinates[0], // 백엔드 좌표 형식 [longitude, latitude]에 맞춰 매핑
            locY: place.location.coordinates[1],
            address: place.address, // 주소 정보 추가
            rating: place.rating, // 평점 정보 추가 (있는 경우)
          })),
