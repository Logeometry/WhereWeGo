...
해당 라우터는 쿠키에 저장된 user의 정보를 불러옵니다.
...
로그인되지 않은 상태로 `/me` API를 호출하면, `401` status_code가 반환


## 📑 구글 로그인 라우터에서 발생하는 상태 코드

상태코드를 참고해서 나중에 오류창도 만들면 좋을 것같아서 간단하게 정리해서 추가해둡니다 .

| 상태 코드 | 설명 |
|-----------|------|
| `400` | `code` 파라미터가 누락되어 구글 로그인 콜백이 실패한 경우. |
| `401` | 인증되지 않은 요청입니다. (예: `access_token`이 없거나 유효하지 않은 토큰) |
